...
const normalizeRole = (user) => String(user?.role || user?.user?.role || "buyer").trim().toLowerCase();
...
export default function Checkout() {
  ...
  const pay = async () => {
    ...
    if (customerRestrictedRoles.has(normalizeRole(user))) {
  ...
  const isRestrictedAccount = Boolean(user && customerRestrictedRoles.has(normalizeRole(user)));
  return (
    <main className="min-h-[70vh] bg-slate-50 dark:bg-slate-950">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        ...
        {!user && <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-blue-200 bg-blue-50 p-5 dark:border-blue-900/60 dark:bg-blue-950/30 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-black text-blue-950 dark:text-blue-100">Please log in to complete your purchase</p><p className="mt-1 text-sm text-blue-800 dark:text-blue-300">Your cart is saved. Sign in with your buyer account to continue securely.</p></div><button type="button" onClick={goToLogin} className="shrink-0 rounded-xl bg-blue-600 px-5 py-3 text-sm font-black text-white hover:bg-blue-700">Log in to continue</button></div>}
        {isRestrictedAccount && <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 dark:border-amber-900/60 dark:bg-amber-950/30"><p className="font-black text-amber-950 dark:text-amber-100">Buyer account required</p><p className="mt-1 text-sm text-amber-800 dark:text-amber-300">Seller and administrator accounts manage the marketplace and cannot place customer purchases.</p></div>}
