  ...
  useEffect(() => {
    ...
    const loadBooks = async () => {
      try {
        ...
        const data = Array.isArray(response?.data) ? response.data : Array.isArray(response?.data?.books) ? response.data.books : Array.isArray(response?.data?.data) ? response.data.data : [];
        if (active) { setBooks(data); setError(false); }
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-blue-950 to-blue-800 text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[1.15fr_.85fr] lg:items-center lg:px-10 lg:py-28">
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-3"><Link to="/books" className="rounded-xl bg-white px-6 py-3 font-bold text-slate-950 transition hover:bg-blue-50">Browse marketplace</Link><Link to="/seller/register" className="rounded-xl border border-white/30 bg-white/10 px-6 py-3 font-bold text-white backdrop-blur transition hover:bg-white/20">Become a seller</Link></div>
          </div>
          <div className="hidden lg:block"><div className="relative mx-auto max-w-md rounded-[2rem] border border-white/20 bg-white/10 p-5 shadow-2xl backdrop-blur"><div className="rounded-[1.5rem] bg-white p-6 text-slate-900"><div className="flex items-center justify-between"><span className="text-sm font-bold text-blue-700">BOOKHUB</span><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-700">Trusted marketplace</span></div><div className="mt-7 grid grid-cols-2 gap-4"><div className="rounded-2xl bg-blue-50 p-5"><div className="text-3xl">📚</div><p className="mt-3 font-bold">Great reads</p></div><div className="rounded-2xl bg-slate-100 p-5"><div className="text-3xl">🛍️</div><p className="mt-3 font-bold">Easy checkout</p></div><div className="rounded-2xl bg-amber-50 p-5"><div className="text-3xl">⭐</div><p className="mt-3 font-bold">Quality listings</p></div><div className="rounded-2xl bg-emerald-50 p-5"><div className="text-3xl">🇰🇪</div><p className="mt-3 font-bold">Made for Kenya</p></div></div></div></div></div>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-10"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="font-bold uppercase tracking-widest text-blue-600">Explore</p><h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Find books for every reader</h2><p className="mt-3 max-w-2xl text-slate-600 dark:text-slate-300">Start with a category or search the full marketplace.</p></div><Link to="/books" className="font-bold text-blue-600 hover:text-blue-700">View all books →</Link></div><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{categories.map((category) => <Link key={category.name} to={`/books?category=${encodeURIComponent(category.name)}`} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900"><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 text-2xl dark:bg-blue-950">{category.icon}</div><h3 className="mt-5 text-lg font-bold">{category.name}</h3><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">{category.description}</p><span className="mt-4 inline-block text-sm font-bold text-blue-600 transition group-hover:translate-x-1">Explore →</span></Link>)}</div></section>
